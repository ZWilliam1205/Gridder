...
public class Gridder extends javax.swing.JFrame 
    ...
    private void jButton7ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton11ActionPerformed
        ...
        int[][] temporaryGrid = new int[gridCount][gridCount];

        for(int i = 0; i < 100; i++) {
            for(int j = 0; j < 100; j++) {
                temporaryGrid[i][j] = grid[i][j];
            }
        }
        ...
        for(int i = 0; i < 100; i++){
            for(int i2 = 0; i2 < 100; i2++){
                int count = 0;
                if(temporaryGrid[i][i2] == 0){
                    for(int i3 = i-1 ; i3 <= i+1 ; i3++){
                        for(int j = i2-1; j<= i2+1 ; j++){
                            if(i3>=0 && i3<=99 && j>=0 && j<=99){
                                if(temporaryGrid[i3][j]==1){
                                    count++;
                                }
                            }
                        }
